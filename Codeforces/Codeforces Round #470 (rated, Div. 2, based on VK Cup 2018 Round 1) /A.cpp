...
bool isValid(int i, int j){
    return (i>=0 && i<r && j>=0 && j<c && arr[i][j]!='D' && !visited[i][j]);
}

void dfs(auto& arr, int i, int j, auto& visited){
    visited[i][j] = 1;
    if(arr[i][j]=='S'){
        // cout<<i<<j;
        yes = 0;
    ...
    if(isValid(i-1,j))  dfs(arr,i-1,j,visited);
    if(isValid(i+1,j))  dfs(arr,i+1,j,visited);
    if(isValid(i,j-1))  dfs(arr,i,j-1,visited);
    if(isValid(i,j+1))  dfs(arr,i,j+1,visited);
}
